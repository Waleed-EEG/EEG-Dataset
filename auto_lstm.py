...
def augment_with_gaussian_noise(X, y, noise_std=0.01, augment_factor=1):
    """
    Augment samples with label 1 by adding Gaussian noise.
    Parameters:
        - noise_std: standard deviation of Gaussian noise
        - augment_factor: how many noisy copies to generate per sample
    """
    ...
    print(f" - Stress instances (label=1): {num_stress}")
    print(f" - Non-stress instances (label=0): {original_total - num_stress}")

    # Augmentation
    X_aug = []
    y_aug = []
    for _ in range(augment_factor):
        noise = np.random.normal(0, noise_std, size=X[stress_indices].shape)
        X_noised = X[stress_indices] + noise
        X_aug.append(X_noised)
        y_aug.append(y[stress_indices])
    ...
    print(f" - Total instances: {len(y_combined)}")
    print(f" - Stress instances (label=1): {np.sum(y_combined == 1)}")
    print(f" - Non-stress instances (label=0): {np.sum(y_combined == 0)}")
...
class LSTMWithAttention(nn.Module):
    def __init__(self, input_dim, hidden_dim=128, num_layers=1, num_classes=1):  # Added num_classes as a parameter
        super().__init__()
        self.lstm = nn.LSTM(input_dim, hidden_dim, num_layers, batch_first=True, bidirectional=True)
        self.attention = nn.Linear(hidden_dim * 2, 1)  # Attention layer
        self.fc = nn.Linear(hidden_dim * 2, num_classes)  # Output layer with num_classes units
        self.dropout = nn.Dropout(0.3)

    def forward(self, x):
        h_lstm, _ = self.lstm(x)  # h_lstm: [B, T, 2*H]
        attn_weights = torch.softmax(self.attention(h_lstm), dim=1)  # [B, T, 1]
        context = torch.sum(attn_weights * h_lstm, dim=1)  # [B, 2*H]
        out = self.dropout(context)
        return self.fc(out)  # Output with num_classes units
...
def load_all_data():
    ...
    for i in range(1, NUM_SUBJECTS + 1):
        ...
        if not set(EXPECTED_COLUMNS + RAW_COLUMNS + ['Label']).issubset(df.columns):
            print(f"Missing columns in file {file_path}")
            continue

        # Convert labels: 0 is non-stress, 1 and 2 are stress
        ...
        save_features(X, y, i)
        # Augment stress class with Gaussian noise
        X, y = augment_with_gaussian_noise(X, y, noise_std=0.01, augment_factor=1)
        data_per_subject.append((X, y))
...
# === TRAINING + EVALUATION ===
def train_and_evaluate(X_train, y_train, X_test, y_test, input_dim, num_classes=2, device='cpu'):
    ...
    classifier = LSTMWithAttention(input_dim=X_train_encoded.shape[2], num_classes=1).to(device)
    cls_optimizer = optim.Adam(classifier.parameters(), lr=0.001)
    cls_criterion = nn.BCEWithLogitsLoss()  # Changed to BCEWithLogitsLoss for binary classification
    ...
    for epoch in range(10):  # Adjust as needed
        ...
        for X_batch, y_batch in train_loader_cls:
            ...
            loss = cls_criterion(outputs.squeeze(dim=1), y_batch.float())  # Ensure y_batch is float for BCEWithLogitsLoss
    ...
    with torch.no_grad():
        for X_batch, y_batch in test_loader_cls:
            X_batch = X_batch.to(device)
            outputs = classifier(X_batch)
            predicted = (torch.sigmoid(outputs).squeeze(dim=1) > 0.5).int()  # Convert logits to binary prediction
